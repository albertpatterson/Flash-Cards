...
let els =
    document.querySelectorAll('[data-nav], [data-action], [data-actionsync]');
[].forEach.call(els, el => {
...
function getErrorWrapper(actionEl) {
  ...
  while (checkEl && !checkEl.classList.contains('modal-content'))
    checkEl = checkEl.parentElement;
  if (checkEl) {
    let errorWrapper = document.querySelector(`[data-error="${actionName}"]`);
    if (!errorWrapper)
      console.warn('No error wrapper found for action ' + actionName);
    return errorWrapper;
  } else {
...
function disableActionAndNav() {
  [].forEach.call(
      document.querySelectorAll('[data-action], [data-nav]'),
      el => {el.disabled = true});
}
